...
void TEST_func_scaleAlignment() {
  ...
  printf("TEST: %u | %u | %u | %u --- %d\n", TEST_1.bits[0], TEST_1.bits[1],
         TEST_1.bits[2], TEST_1.bits[3], getSign(TEST_1));
  // printf("TEST: %u | %u | %u | %u --- %d\n\n", TEST_2.bits[0],
  // TEST_2.bits[1], TEST_2.bits[2], TEST_2.bits[3], getSign(TEST_2));

  int status = s21_sub(TEST_1, res);

  printf("TEST: %u | %u | %u | %u = %d\n\n", res.bits[0], res.bits[1],
         res.bits[2], res.bits[3], status);
  ...
  // printf("TEST: %d | %d | %u | %u\n", TEST_1.bits[0], TEST_1.bits[1],
  // TEST_1.bits[2], TEST_1.bits[3]); printf("TEST: %d | %d | %u | %u\n\n",
